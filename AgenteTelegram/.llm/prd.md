...
1. **Chat livre** — responde qualquer pergunta sobre o e-commerce consultando o banco em tempo real via tool use (o LLM executa SQL dinamicamente).
...
**LLM:** Groq API (https://console.groq.com/keys) com tool use para execução de SQL — modelo hospedado na Groq com suporte a function/tool calling
...
Atualize no .env as variaveis que preciso passar: `GROQ_API_KEY` (Groq) e `TELEGRAM_BOT_TOKEN` (Telegram)